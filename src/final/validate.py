...
def main() -> None:
    ...
    for index in range(total_files):
        ...
        lr_y_tensor = imgproc.image2tensor(
            lr_y_image, range_norm=False, half=True).to(config.device).unsqueeze_(0)
        hr_y_tensor = imgproc.image2tensor(
            hr_y_image, range_norm=False, half=True).to(config.device).unsqueeze_(0)
        ...
        # Cal PSNR
        total_psnr += 10. * \
            torch.log10(1. / torch.mean((sr_y_tensor - hr_y_tensor) ** 2))

        # Save image
        sr_y_image = imgproc.tensor2image(
            sr_y_tensor, range_norm=False, half=True)
